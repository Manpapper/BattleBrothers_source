local gt = this.getroottable();

if (!("World" in gt.Const))
{
	gt.Const.World <- {};
}

if (!("Spawn" in gt.Const.World))
{
	gt.Const.World.Spawn <- {};
}

gt.Const.World.Spawn.Direwolves <- [
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 3
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 3
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 4
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 5
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 6
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 7
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 8
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 9
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 10
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 11
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 12
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 13
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 14
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 15
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 16
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 17
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 18
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 19
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 20
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 21
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 22
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 23
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 24
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 1
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 1
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 2
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 4
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 5
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 7
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 4
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 8
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 8
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 8
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 4
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 10
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 12
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 13
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 14
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 8
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 13
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 13
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 7
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 16
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 16
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 15
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 17
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 18
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 19
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 20
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 21
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 22
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 23
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 24
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 25
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_werewolf_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 26
			}
		]
	}
];
gt.Const.World.Spawn.Ghouls <- [
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 5
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 6
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 8
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 9
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 9
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 2
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 4
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 12
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 5
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 15
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 5
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 1
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 1
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 9
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 1
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 11
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 2
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 11
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 15
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 16
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 3
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 4
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 1
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 1
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 14
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 1
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 9
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 14
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 15
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 11
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 21
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 9
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 5
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 6
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 7
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 9
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 9
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 7
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 8
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 9
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 9
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 14
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 4
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 16
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 9
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 3
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 16
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 12
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 20
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 4
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 23
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 5
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 25
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 3
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 11
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 27
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 2
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 9
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 3
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_ghoul_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 11
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 7
			}
		]
	}
];
gt.Const.World.Spawn.Lindwurm <- [
	{
		Cost = 0,
		MovementSpeedMult = 0.75,
		Body = "figure_lindwurm_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Lindwurm,
				Num = 1
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 0.75,
		Body = "figure_lindwurm_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Lindwurm,
				Num = 1
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 0.75,
		Body = "figure_lindwurm_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Lindwurm,
				Num = 2
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 0.75,
		Body = "figure_lindwurm_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Lindwurm,
				Num = 2
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 0.75,
		Body = "figure_lindwurm_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Lindwurm,
				Num = 3
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 0.75,
		Body = "figure_lindwurm_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Lindwurm,
				Num = 3
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 0.75,
		Body = "figure_lindwurm_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Lindwurm,
				Num = 4
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 0.75,
		Body = "figure_lindwurm_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Lindwurm,
				Num = 5
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 0.75,
		Body = "figure_lindwurm_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Lindwurm,
				Num = 6
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 0.75,
		Body = "figure_lindwurm_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Lindwurm,
				Num = 7
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 0.75,
		Body = "figure_lindwurm_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Lindwurm,
				Num = 8
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 0.75,
		Body = "figure_lindwurm_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Lindwurm,
				Num = 9
			}
		]
	}
];
gt.Const.World.Spawn.Unhold <- [
	{
		Body = "figure_unhold_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 1
			}
		]
	},
	{
		Body = "figure_unhold_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 2
			}
		]
	},
	{
		Body = "figure_unhold_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 3
			}
		]
	},
	{
		Body = "figure_unhold_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 4
			}
		]
	},
	{
		Body = "figure_unhold_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 5
			}
		]
	},
	{
		Body = "figure_unhold_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 6
			}
		]
	},
	{
		Body = "figure_unhold_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 7
			}
		]
	},
	{
		Body = "figure_unhold_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 8
			}
		]
	},
	{
		Body = "figure_unhold_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 9
			}
		]
	},
	{
		Body = "figure_unhold_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 10
			}
		]
	},
	{
		Body = "figure_unhold_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 11
			}
		]
	},
	{
		Body = "figure_unhold_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 12
			}
		]
	}
];
gt.Const.World.Spawn.UnholdFrost <- [
	{
		Body = "figure_unhold_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdFrost,
				Num = 1
			}
		]
	},
	{
		Body = "figure_unhold_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdFrost,
				Num = 2
			}
		]
	},
	{
		Body = "figure_unhold_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdFrost,
				Num = 3
			}
		]
	},
	{
		Body = "figure_unhold_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdFrost,
				Num = 4
			}
		]
	},
	{
		Body = "figure_unhold_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdFrost,
				Num = 5
			}
		]
	},
	{
		Body = "figure_unhold_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdFrost,
				Num = 6
			}
		]
	},
	{
		Body = "figure_unhold_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdFrost,
				Num = 7
			}
		]
	},
	{
		Body = "figure_unhold_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdFrost,
				Num = 8
			}
		]
	},
	{
		Body = "figure_unhold_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdFrost,
				Num = 9
			}
		]
	},
	{
		Body = "figure_unhold_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdFrost,
				Num = 10
			}
		]
	},
	{
		Body = "figure_unhold_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdFrost,
				Num = 11
			}
		]
	},
	{
		Body = "figure_unhold_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdFrost,
				Num = 12
			}
		]
	}
];
gt.Const.World.Spawn.UnholdBog <- [
	{
		Body = "figure_unhold_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 1
			}
		]
	},
	{
		Body = "figure_unhold_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 2
			}
		]
	},
	{
		Body = "figure_unhold_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 3
			}
		]
	},
	{
		Body = "figure_unhold_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 4
			}
		]
	},
	{
		Body = "figure_unhold_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 5
			}
		]
	},
	{
		Body = "figure_unhold_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 6
			}
		]
	},
	{
		Body = "figure_unhold_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 7
			}
		]
	},
	{
		Body = "figure_unhold_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 8
			}
		]
	},
	{
		Body = "figure_unhold_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 9
			}
		]
	},
	{
		Body = "figure_unhold_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 10
			}
		]
	},
	{
		Body = "figure_unhold_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 11
			}
		]
	},
	{
		Body = "figure_unhold_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 12
			}
		]
	}
];
gt.Const.World.Spawn.Spiders <- [
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 5
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 6
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 7
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 8
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 9
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 10
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 11
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 12
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 13
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 14
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 15
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 16
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 17
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 18
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 19
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 20
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 21
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 22
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 23
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 24
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 25
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 26
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 27
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 28
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 29
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 30
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 31
			}
		]
	},
	{
		Body = "figure_spider_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 32
			}
		]
	}
];
gt.Const.World.Spawn.Alps <- [
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 3
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 4
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 5
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 6
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 7
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 8
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 9
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 10
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 11
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 12
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 13
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 14
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 3
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 4
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 5
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 6
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 7
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 8
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 9
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 10
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 11
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 12
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 13
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 14
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 3
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 4
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 5
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 6
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 7
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 8
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 9
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 10
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 11
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 12
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 13
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 14
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 1
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 1
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 2
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 3
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 2
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 2
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 9
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 3
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 4
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 1
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 5
			}
		]
	},
	{
		Body = "figure_alp_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Alp,
				Num = 11
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 6
			}
		]
	}
];
gt.Const.World.Spawn.Schrats <- [
	{
		Body = "figure_schrat_01",
		MovementSpeedMult = 0.5,
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 1
			}
		]
	},
	{
		Body = "figure_schrat_01",
		MovementSpeedMult = 0.5,
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 2
			}
		]
	},
	{
		Body = "figure_schrat_01",
		MovementSpeedMult = 0.5,
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 3
			}
		]
	},
	{
		Body = "figure_schrat_01",
		MovementSpeedMult = 0.5,
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 4
			}
		]
	},
	{
		Body = "figure_schrat_01",
		MovementSpeedMult = 0.5,
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 5
			}
		]
	},
	{
		Body = "figure_schrat_01",
		MovementSpeedMult = 0.5,
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 6
			}
		]
	},
	{
		Body = "figure_schrat_01",
		MovementSpeedMult = 0.5,
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 7
			}
		]
	},
	{
		Body = "figure_schrat_01",
		MovementSpeedMult = 0.5,
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 8
			}
		]
	},
	{
		Body = "figure_schrat_01",
		MovementSpeedMult = 0.5,
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 9
			}
		]
	},
	{
		Body = "figure_schrat_01",
		MovementSpeedMult = 0.5,
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 10
			}
		]
	},
	{
		Body = "figure_schrat_01",
		MovementSpeedMult = 0.5,
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 11
			}
		]
	},
	{
		Body = "figure_schrat_01",
		MovementSpeedMult = 0.5,
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 12
			}
		]
	},
	{
		Body = "figure_schrat_01",
		MovementSpeedMult = 0.5,
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 13
			}
		]
	},
	{
		Body = "figure_schrat_01",
		MovementSpeedMult = 0.5,
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 14
			}
		]
	},
	{
		Body = "figure_schrat_01",
		MovementSpeedMult = 0.5,
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 15
			}
		]
	},
	{
		Body = "figure_schrat_01",
		MovementSpeedMult = 0.5,
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 16
			}
		]
	},
	{
		Body = "figure_schrat_01",
		MovementSpeedMult = 0.5,
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 17
			}
		]
	},
	{
		Body = "figure_schrat_01",
		MovementSpeedMult = 0.5,
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 18
			}
		]
	}
];
gt.Const.World.Spawn.HexenAndMore <- [
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BanditRaider,
				Num = 4
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BanditRaider,
				Num = 6
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BanditRaider,
				Num = 4
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BanditRaider,
				Num = 7
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BanditRaider,
				Num = 10
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BanditRaider,
				Num = 9
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BanditRaider,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.BanditMarksman,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.SpiderBodyguard,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 7
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.SpiderBodyguard,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 10
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.SpiderBodyguard,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 12
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.SpiderBodyguard,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 8
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.SpiderBodyguard,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 11
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 14
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.SpiderBodyguard,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 10
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.SpiderBodyguard,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 15
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.SpiderBodyguard,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 16
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.SpiderBodyguard,
				Num = 5
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.SpiderBodyguard,
				Num = 6
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.SpiderBodyguard,
				Num = 8
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 9
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.SpiderBodyguard,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 7
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.SpiderBodyguard,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.Spider,
				Num = 10
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 9
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 10
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 2
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 3
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 2
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 3
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 2
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 4
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 5
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 6
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 8
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 4
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 5
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 6
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 3
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 4
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 3
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 6
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 6
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 8
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 5
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 7
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 7
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 3
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 5
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 3
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 4
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 3
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 2
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 3
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 3
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 4
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 2
			}
		]
	}
];
gt.Const.World.Spawn.HexenAndNoSpiders <- [
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BanditRaider,
				Num = 4
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BanditRaider,
				Num = 6
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BanditRaider,
				Num = 4
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BanditRaider,
				Num = 7
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BanditRaider,
				Num = 10
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BanditRaider,
				Num = 9
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BanditRaider,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.BanditMarksman,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 9
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 10
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 2
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Ghoul,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulHIGH,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.GhoulLOW,
				Num = 3
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 3
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 2
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 3
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 2
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 4
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.Unhold,
				Num = 2
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 6
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 8
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 4
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 5
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 6
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 3
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 4
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 3
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 6
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 6
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 8
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 5
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 7
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 7
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 5
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 3
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 4
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Direwolf,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfBodyguard,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.DirewolfHIGH,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.UnholdBog,
				Num = 3
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 2
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 3
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 3
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Schrat,
				Num = 4
			}
		]
	}
];
gt.Const.World.Spawn.Hexen <- [
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 1
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 2
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 3
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 4
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 5
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 6
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 7
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 8
			}
		]
	},
	{
		Body = "figure_hexe_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hexe,
				Num = 9
			}
		]
	}
];
gt.Const.World.Spawn.Kraken <- [
	{
		MovementSpeedMult = 0.0,
		VisibilityMult = 0.5,
		VisionMult = 1.0,
		Body = "figure_kraken_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Kraken,
				Num = 1
			}
		]
	}
];
gt.Const.World.Spawn.Hyenas <- [
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 3
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 3
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 4
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 5
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 6
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 7
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 8
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 9
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 10
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 11
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 12
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 13
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 14
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 15
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 16
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 17
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 18
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 19
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 20
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 21
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 22
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 23
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 24
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 1
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 1
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 2
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 4
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 5
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 7
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 4
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 8
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 8
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 8
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 4
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 10
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 12
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 13
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 14
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 8
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 13
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 13
			},
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 7
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 16
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 16
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Hyena,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 15
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 17
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 18
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 19
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 20
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 21
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 22
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 23
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 24
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 25
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_hyena_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.HyenaHIGH,
				Num = 26
			}
		]
	}
];
gt.Const.World.Spawn.Serpents <- [
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 3
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 3
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 4
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 5
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 6
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 7
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 8
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 9
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 10
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 11
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 12
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 13
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 14
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 15
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 16
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 17
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 18
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 19
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 20
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 21
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 22
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 23
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 24
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 25
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 26
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 27
			}
		]
	},
	{
		Cost = 0,
		MovementSpeedMult = 1.0,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_serpent_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.Serpent,
				Num = 28
			}
		]
	}
];
gt.Const.World.Spawn.SandGolems <- [
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 3
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 6
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 9
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 10
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 11
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 12
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 13
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 14
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 15
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 16
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 17
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 18
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 19
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 20
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 21
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 22
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 23
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 24
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 25
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 26
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 27
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 28
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 29
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 30
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 31
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 32
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 33
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 34
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 35
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 36
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 37
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 38
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 39
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 40
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 41
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 42
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 4
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 5
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 6
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 6
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 7
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 8
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 9
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 10
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 11
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 12
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 13
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 14
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 15
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 16
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 17
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 18
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 19
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 20
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 21
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 22
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 23
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 24
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 25
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 10
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 12
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 14
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 16
			}
		]
	},
	{
		MovementSpeedMult = 0.9,
		VisibilityMult = 1.0,
		VisionMult = 1.0,
		Body = "figure_golem_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.SandGolemMEDIUM,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.SandGolem,
				Num = 18
			}
		]
	}
];
this.Const.World.Spawn.Beasts <- [];
this.Const.World.Spawn.Beasts.extend(this.Const.World.Spawn.Direwolves);
this.Const.World.Spawn.Beasts.extend(this.Const.World.Spawn.Ghouls);
function onCostCompare( _t1, _t2 )
{
	if (_t1.Cost < _t2.Cost)
	{
		return -1;
	}
	else if (_t1.Cost > _t2.Cost)
	{
		return 1;
	}

	return 0;
}

function calculateCosts( _p )
{
	foreach( p in _p )
	{
		p.Cost <- 0;

		foreach( t in p.Troops )
		{
			p.Cost += t.Type.Cost * t.Num;
		}

		if (!("MovementSpeedMult" in p))
		{
			p.MovementSpeedMult <- 1.0;
		}

		if (!("VisibilityMult" in p))
		{
			p.VisibilityMult <- 1.0;
		}

		if (!("VisionMult" in p))
		{
			p.VisionMult <- 1.0;
		}
	}

	_p.sort(this.onCostCompare);
}

this.calculateCosts(this.Const.World.Spawn.Direwolves);
this.calculateCosts(this.Const.World.Spawn.Ghouls);
this.calculateCosts(this.Const.World.Spawn.Lindwurm);
this.calculateCosts(this.Const.World.Spawn.Unhold);
this.calculateCosts(this.Const.World.Spawn.UnholdFrost);
this.calculateCosts(this.Const.World.Spawn.UnholdBog);
this.calculateCosts(this.Const.World.Spawn.Spiders);
this.calculateCosts(this.Const.World.Spawn.Schrats);
this.calculateCosts(this.Const.World.Spawn.Alps);
this.calculateCosts(this.Const.World.Spawn.Hexen);
this.calculateCosts(this.Const.World.Spawn.HexenAndMore);
this.calculateCosts(this.Const.World.Spawn.HexenAndNoSpiders);
this.calculateCosts(this.Const.World.Spawn.Kraken);
this.calculateCosts(this.Const.World.Spawn.Hyenas);
this.calculateCosts(this.Const.World.Spawn.Serpents);
this.calculateCosts(this.Const.World.Spawn.SandGolems);
this.calculateCosts(this.Const.World.Spawn.Beasts);

