local gt = this.getroottable();

if (!("World" in gt.Const))
{
	gt.Const.World <- {};
}

if (!("Spawn" in gt.Const.World))
{
	gt.Const.World.Spawn <- {};
}

gt.Const.World.Spawn.Barbarians <- [
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 5
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 6
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 7
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 8
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 9
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 10
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 11
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 12
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 13
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 14
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 15
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 15
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 16
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 17
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 18
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 5
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 6
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 7
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 8
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 9
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 10
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 11
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 12
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 13
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 14
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 15
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 16
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 13
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 15
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 16
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 17
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 18
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 3
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 11
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 12
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 5
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 13
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 6
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 14
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 6
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 5
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 6
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 7
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 8
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 9
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 10
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 11
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 12
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 13
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 14
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 14
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 15
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 16
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 17
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 18
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 3
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 5
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 6
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 7
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 8
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 3
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 12
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 3
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 3
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 12
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 5
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 5
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 6
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 6
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 9
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 9
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 11
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 12
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 13
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 14
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 15
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 3
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 5
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 6
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 6
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 7
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 8
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 9
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 10
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 12
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 3
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 12
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 12
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 11
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 12
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 13
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 3
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 12
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 3
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 12
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 3
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 3
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 11
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 12
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 13
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 5
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 7
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 8
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 8
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 10
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 10
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 12
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 12
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 12
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 10
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 6
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 6
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 3
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 8
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 12
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 14
			}
		]
	},
	{
		Body = "figure_wildman_02",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 10
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 10
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 5
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 1
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 10
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 1
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 12
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 6
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 14
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 5
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 14
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 13
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 7
			}
		]
	},
	{
		Body = "figure_wildman_03",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnhold,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianUnholdFrost,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianBeastmaster,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianDrummer,
				Num = 2
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianMarauder,
				Num = 9
			},
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChampion,
				Num = 9
			}
		]
	}
];
gt.Const.World.Spawn.BarbarianHunters <- [
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 3
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 4
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 5
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 2
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 3
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 6
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 3
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 7
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 8
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 9
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 4
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 10
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 5
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 11
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 5
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 11
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 6
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 12
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 5
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 12
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 6
			}
		]
	},
	{
		Body = "figure_wildman_01",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianThrall,
				Num = 12
			},
			{
				Type = this.Const.World.Spawn.Troops.Warhound,
				Num = 7
			}
		]
	}
];
gt.Const.World.Spawn.BarbarianKing <- [
	{
		Body = "figure_wildman_06",
		Troops = [
			{
				Type = this.Const.World.Spawn.Troops.BarbarianChosen,
				Num = 1
			}
		]
	}
];
this.Const.World.Spawn.Barbarians.extend(this.Const.World.Spawn.BarbarianHunters);
function onCostCompare( _t1, _t2 )
{
	if (_t1.Cost < _t2.Cost)
	{
		return -1;
	}
	else if (_t1.Cost > _t2.Cost)
	{
		return 1;
	}

	return 0;
}

function calculateCosts( _p )
{
	foreach( p in _p )
	{
		p.Cost <- 0;

		foreach( t in p.Troops )
		{
			p.Cost += t.Type.Cost * t.Num;
		}

		if (!("MovementSpeedMult" in p))
		{
			p.MovementSpeedMult <- 1.0;
		}

		if (!("VisibilityMult" in p))
		{
			p.VisibilityMult <- 1.0;
		}

		if (!("VisionMult" in p))
		{
			p.VisionMult <- 1.0;
		}
	}

	_p.sort(this.onCostCompare);
}

this.calculateCosts(this.Const.World.Spawn.Barbarians);
this.calculateCosts(this.Const.World.Spawn.BarbarianHunters);
this.calculateCosts(this.Const.World.Spawn.BarbarianKing);

