this.tactical_state <- this.inherit("scripts/states/state", {
	...
	function tactical_combat_result_screen_onQueryCombatInformation()
	{
		...
		case this.Const.Tactical.CombatResult.EnemyDestroyed:
			...
			result.subTitle = "The enemy was destroyed in " + rounds + " round" + (rounds > 1 ? "s" : "");
			break;

		case this.Const.Tactical.CombatResult.EnemyRetreated:
			result.title = "Victory";
			result.subTitle = "The enemy retreated after " + rounds + " round" + (rounds > 1 ? "s" : "");
			break;

		case this.Const.Tactical.CombatResult.PlayerDestroyed:
			result.title = "Defeat";
			result.subTitle = "You lost after " + rounds + " round" + (rounds > 1 ? "s" : "");
			break;

		case this.Const.Tactical.CombatResult.PlayerRetreated:
			result.title = "Retreat";
			result.subTitle = "You retreated after " + rounds + " round" + (rounds > 1 ? "s" : "");
	...
	function showFleeScreen( _tag = null )
	{
		...
		this.Tooltip.hide();
		this.m.TacticalScreen.hide();
		this.m.TacticalDialogScreen.show("Retreat From Combat", "Are you sure?", this.Const.Strings.UI.FleeDialogueConsequences, "Retreat!", "Cancel", this.tactical_flee_screen_onFleePressed.bindenv(this), this.tactical_flee_screen_onCancelPressed.bindenv(this));
		this.Cursor.setCursor(this.Const.UI.Cursor.Hand);
		this.m.MenuStack.push(function ()
		{
			this.Cursor.setCursor(this.Const.UI.Cursor.Hand);
	...
	function showRetreatScreen( _tag = null )
	{
		...
		this.Tooltip.hide();
		this.m.TacticalScreen.hide();
		this.m.TacticalDialogScreen.show("The Enemy Retreats", "", this.Const.Strings.UI.RetreatDialogueConsequences, "It\'s over", "Run them down!", this.tactical_retreat_screen_onYesPressed.bindenv(this), this.tactical_retreat_screen_onNoPressed.bindenv(this));
		this.Cursor.setCursor(this.Const.UI.Cursor.Hand);
		this.m.MenuStack.push(function ()
		{
			this.Cursor.setCursor(this.Const.UI.Cursor.Hand);
