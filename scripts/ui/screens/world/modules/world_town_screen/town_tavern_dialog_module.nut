this.town_tavern_dialog_module <- this.inherit("scripts/ui/screens/ui_module", {
	...
	function queryData()
	{
		return {
			...
			RightInfo = "Pay a round for your men to get their spirits up ([img]gfx/ui/tooltips/money.png[/img]" + this.Math.round(this.World.getPlayerRoster().getSize() * 5 * this.m.Tavern.getSettlement().getBuyPriceMult()) + ").",
			Assets = this.m.Parent.queryAssetsInformation()
		};
	}
