this.conquer_holy_site_southern_contract <- this.inherit("scripts/contracts/contract", {
	...
	function createScreens()
	{
		...
		this.m.Screens.push({
			ID = "Failure",
			Title = "After the battle...",
			Text = "[img]gfx/ui/events/%illustration%.png[/img]{You\'ve failed to protect %holysite% from the northerners. There\'s no reason to stick around, and the only reason to return to %employer% is if you want your head on one of the Vizier\'s gold-encrusted platters.}",
			Image = "",
			Characters = [],
			List = [],
			Options = [
