	...
	function createStates()
	{
		...
		this.m.States.push({
			...
			function start()
			{
				...
				if (this.Contract.m.Payment.Count != 0)
				{
					...
					this.Contract.m.BulletpointsObjectives.push("Get paid for every head of an attacker you slay (%killcount%/%maxcount%)");
				}
		...
		this.m.States.push({
			...
			function start()
			{
				...
				this.Contract.m.BulletpointsObjectives = [
					"Return %noble% safely to %noblesettlement% in the %nobledirection%"
				];
