this.hold_chokepoint_contract <- this.inherit("scripts/contracts/contract", {
	...
	function createScreens()
	{
		...
		this.m.Screens.push({
			ID = "Ultimatum3",
			Title = "As you wait...",
			Text = "[img]gfx/ui/events/event_90.png[/img]{You tell the herald to return to his commander. He nods.%SPEECH_ON%May thine fortitude impress the old gods, for it will not impress the might of the North.%SPEECH_OFF%The herald bows and makes his leave.}",
			Image = "",
			List = [],
			Options = [
				{
	...
	function spawnAllies()
	{
		...
		local party = cityState.spawnEntity(tiles[0].Tile, "Regiment of " + cityState.getNameOnly(), true, this.Const.World.Spawn.Southern, this.Math.rand(100, 150) * this.getDifficultyMult() * this.getScaledDifficultyMult(), this.getMinibossModifier());
