this.restore_location_contract <- this.inherit("scripts/contracts/contract", {
	...
	function createStates()
	{
		...
		this.m.States.push({
			...
			{
				this.Contract.m.BulletpointsObjectives = [
					"Escort workers to the %location% near %townname%"
				];
				this.Contract.m.Location.getSprite("selection").Visible = true;
	...
	function spawnCaravan()
	{
		...
		local party = faction.spawnEntity(this.m.Home.getTile(), "Worker Caravan", false, this.Const.World.Spawn.CaravanEscort, this.m.Home.getResources() * 0.4, this.getMinibossModifier());
